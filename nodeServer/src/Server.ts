...
app.get("/record/scores",AuthMiddleware, async (req:Request, res:Response) =>
{
...
app.post("/login", async (req:Request, res:Response) =>
{
    ...
    let [row, fieldInfos] : [InventoryVO[], FieldPacket[]] = await Pool.query(sql, [account, pass]);
    ...
    if(row.length == 1)
     {
        let token:string = JWT.sign(   
         ...
         res.json({ success: true, msg: "로그인 성공", token });
     }

     else
     {
        res.json({success:false, msg:"로그인 실패", token:""});
     }
